...
mainServer <- function(input, output, session, data_reactive, selected_dir_reactive, message_rv, analysis_running_rv) {
  ...
  observeEvent(input$analyze_btn, {
    ...
    tryCatch({
      # Check for skewness to auto-select model
      final_model_choice <- if (input$model_choice == "AutoSelect") {
      ...
      refiner_model <- refineR::findRI(Data = filtered_data[[input$col_value]],
                                       NBootstrap = input$nbootstrap_speed,
      ...
      gender_text <- if (input$col_gender == "") "Combined" else paste0("Gender: ", input$gender_choice)
      ...
      plot_title_rv(paste0("Estimated Reference Intervals for ", input$col_value,
                           model_text,
                           " (", gender_text,
